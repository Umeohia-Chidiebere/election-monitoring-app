...
<v-snackbar v-model="snackbar" timeout="5000">
    ...
    <v-dialog v-model="dialog" fullscreen hide-overlay transition="dialog-bottom-transition">
      ...
      <v-card>
        ...
        <v-toolbar-title>Add Constituency</v-toolbar-title>
          <v-spacer></v-spacer>
        </v-toolbar>

<v-card-text>

<form @submit.prevent="store_data" method="post" class="constituency_zone_data py-5" enctype="multipart/form-data">
<input type="hidden" name="store_constituency" value="1"> 
<v-text-field label="Constituency" name="name" color="green" placeholder="Constituency" required></v-text-field>
<v-select 
    label="Wards"
    name="ward_id"
...
<v-select 
    ...
    color="green"
    :items="app_data.all_admins.constituency"
    item-text="name"
    item-value="id"
    required>
</v-select>
...
<v-card>
                <v-card-title> Constituency({{ app_data.all_constituency.length }}) </v-card-title>
                <v-divider></v-divider>
                <v-card-text>
...
<section class="">
  ...
  <v-list>
          ...
          <v-list-item v-for="zone in app_data.all_constituency" :key="zone.id">
            <v-list-item-content>
              <v-list-item-title> <h6 class="text-success"> {{ `${zone.name} - ${zone.state}`  }} <button v-if="current_user.role.slug == 'super_admin'"  class="delete_data pl-3"><i :id="zone.id" class="fa fa-trash text-danger"></i></button> </h6> </v-list-item-title>
    ...
    export default {
        data(){
            return{
                ...
                is_processing:false, edit_dialog:false, edit_data:{}
            }
        },
       ...
       methods:{
            ...
            store_data(){
                ...
                var data = $('.constituency_zone_data').serialize();
